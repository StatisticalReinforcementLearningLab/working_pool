...
class MyKernel(Kernel):
    ...
    def __init__(self, num_dimensions,user_mat,time_mat, first_mat,gparams, variance_prior=None, offset_prior=None, active_dims=None):
        ...
        self.register_parameter(name="raw_s2", parameter=torch.nn.Parameter(1.0*torch.tensor(1.0)))
        self.register_constraint("raw_s1",constraint= constraints.Positive())
        self.register_constraint("raw_s2",constraint= constraints.Positive())
        
        self.register_parameter(name="raw_s3", parameter=torch.nn.Parameter(1.0*torch.tensor(1.0)))
        self.register_parameter(name="raw_s4", parameter=torch.nn.Parameter(1.0*torch.tensor(1.0)))
        self.register_constraint("raw_s3",constraint= constraints.Positive())
        self.register_constraint("raw_s4",constraint= constraints.Positive())
    ...
    def _set_s2(self, value):
        if not torch.is_tensor(value):
            value = torch.as_tensor(value).to(self.raw_s2)
        self.initialize(raw_s2=self.raw_s2_constraint.inverse_transform(value))
    ...
    @property
    def s4(self):
        return self.raw_s4_constraint.transform(self.raw_s4)
    
    @s4.setter
    def s4(self, value):
        self._set_s4(value)
    
    def _set_s4(self, value):
        if not torch.is_tensor(value):
            value = torch.as_tensor(value).to(self.raw_s4)
        self.initialize(raw_s4=self.raw_s4_constraint.inverse_transform(value))
    ...
    with gpytorch.settings.use_toeplitz(False):
            for i in range(num_iter):
                ...
                #print(type(output))
                    ...
                    if np.isreal(sigma_temp).all() and not np.isnan(covtemp).all() and np.isreal(sigma_v_temp).all() :
                        sigma_u = sigma_temp
...
#print(cov)
#print(sigma_u)
                return {'uparams':sigma_u,'vparams':sigma_v,'cov':cov,'noise':noise,'like':0}
